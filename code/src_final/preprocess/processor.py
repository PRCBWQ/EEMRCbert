...
class BaseProcessor:
    ...
    @staticmethod
    def _example_generator(raw_examples):
        examples = []
        for _ex in raw_examples:
            text = _ex['sentence']
            events = []
            for _event in _ex['golden-event-mentions']:
                # trigger is special singleRole
                trigger = SingleRole(text=_event['trigger']['text'],
                                     role=_event['event_type'],
                                     start=_event['trigger']['start'],
                                     end=_event['trigger']['end'])
                arguements = []
                for arg in _event['arguments']:
                    arguements.append(
                        SingleRole(text=arg['text'],
                                   role=arg['role'],
                                   start=arg['start'],
                                   end=arg['end']))
                # construct events list
                events.append(Event(trigger=trigger,
                                    event_type=_event['event_type'],
                                    arguments=arguements))
            # construct examples
            examples.append(BaseExample(sentence=text,
                                        events=events))
        return examples
